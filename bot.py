...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute(
        "INSERT OR REPLACE INTO files (file_id, user_id, file_name, file_type, tag) VALUES (?, ?, ?, ?, ?)",
        (file_id_db, user_id, file_name, file_obj.__class__.__name__.lower(), tag)
    )
    conn.commit()
...
if __name__ == "__main__":
    import nest_asyncio
    nest_asyncio.apply()  # Fix for event loop issue on Render
